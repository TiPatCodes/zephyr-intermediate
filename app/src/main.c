...
void t_coop_fn(void *p1, void *p2, void *p3)
{
    // while (1) {
        ...
        LOG_DBG("T_Coop_iteration resets");
        i = 0;
        // k_yield();
    // } 

/* 
    Here because we haven't used the continous while loop,
    the threads terminates after yielding.

    If we are using the while it would be essential to use k_msleep(), 
    if not the preemptible threads will never execute.
*/
